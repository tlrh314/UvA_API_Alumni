...
class ContactForm(forms.Form):
    ...
    cc_myself = forms.BooleanField(
        required=False, widget=forms.CheckboxInput(attrs={"class": "checkchoice"})
    )
...
class SelectThesisForm(forms.Form):
    def __init__(self, alumnus, *args, **kwargs):
        ...
        self.fields["which_thesis"].label_from_instance = lambda obj: "%s: %s" % (
            display_thesis_type(obj.type),
            obj.title,
        )
...
class ThesisForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "advisor": autocomplete.ModelSelect2Multiple(
                url="alumnus-autocomplete", attrs={"class": "form-control"}
            )
