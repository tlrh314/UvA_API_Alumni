    ...
    def queryset(self, request, queryset):
        ...
        if value == "today":
            ...
            queryset = queryset.filter(
                survey_info_updated__gte=self.dates_dict["today"]
            )

        if value == "past_7_days":
            queryset = queryset.filter(survey_info_updated__lt=self.dates_dict["now"])
            queryset = queryset.filter(
                survey_info_updated__gte=self.dates_dict["last_week"]
            )

        if value == "this_month":
            queryset = queryset.filter(survey_info_updated__lt=self.dates_dict["now"])
            queryset = queryset.filter(
                survey_info_updated__gte=self.dates_dict["this_month"]
            )

        if value == "this_year":
            queryset = queryset.filter(survey_info_updated__lt=self.dates_dict["now"])
            queryset = queryset.filter(
                survey_info_updated__gte=self.dates_dict["this_year"]
            )

        if value == "over_day_ago":
            queryset = queryset.exclude(
                survey_info_updated__gte=self.dates_dict["day_ago"]
            )

        if value == "over_week_ago":
            queryset = queryset.exclude(
                survey_info_updated__gte=self.dates_dict["week_ago"]
            )

        if value == "over_month_ago":
            queryset = queryset.exclude(
                survey_info_updated__gte=self.dates_dict["month_ago"]
            )

        if value == "over_year_ago":
            queryset = queryset.exclude(
                survey_info_updated__gte=self.dates_dict["year_ago"]
            )
