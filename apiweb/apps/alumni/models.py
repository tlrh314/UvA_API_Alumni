...
class PositionType(models.Model):
    ...
    plural = models.CharField(
        max_length=80,
        blank=True,
        help_text=_("Full plural name, if this is not a simple appended 's'"),
    )
...
class PreviousPosition(models.Model):
    ...
    alumnus = models.ForeignKey(
        "Alumnus", related_name="positions", on_delete=models.CASCADE
    )
    ...
    fte_per_year = JSONField(
        blank=True,
        default=[],
        help_text="Enter mapping from year to fte in valid JSON syntax",
    )
...
class Alumnus(AbstractBaseUser, PermissionsMixin):
    ...
    passed_away = models.BooleanField(
        _("Deceased"),
        blank=True,
        default=False,
        help_text=_(
            "If selected an asterisk will appear by the name of this alumnus on the website."
        ),
    )

    # Personal information
    academic_title = models.ForeignKey(
        AcademicTitle, on_delete=models.SET_NULL, blank=True, null=True
    )
    ...
    birth_date = models.DateField(
        _("birth date"), blank=True, null=True, help_text="Use format: YYYY-MM-DD"
    )
    ...
    mugshot = models.ImageField(
        _("mugshot"),
        upload_to=get_mugshot_location,
    ...
    last_checked = models.DateTimeField(
        ...
        null=True,
        help_text=_(
            "Update this field if you know that the contact information is up-to-date. Use format: YYYY-MM-DD"
        ),
    )
    ...
    show_biography = models.BooleanField(
        _("Show biography on personal page"), blank=True, default=False
    )
    show_facebook = models.BooleanField(
        _("Show facebook on personal page"), blank=True, default=False
    )
    show_linkedin = models.BooleanField(
        _("Show linkedin on personal page"), blank=True, default=False
    )
    show_twitter = models.BooleanField(
        _("Show twitter on personal page"), blank=True, default=False
    )
    show_email = models.BooleanField(
        _("Show email on personal page"), blank=True, default=False
    )
    show_homepage = models.BooleanField(
        _("Show homepage on personal page"), blank=True, default=False
    )

    # Extra field for survey completed
    survey_info_updated = models.DateTimeField(
        _("Date survey info updated"),
        blank=True,
        null=True,
        help_text="Use format: YYYY-MM-DD",
    )
    survey_email_sent = models.DateTimeField(
        _("Date survey email sent"),
        blank=True,
        null=True,
        help_text="Use format: YYYY-MM-DD",
    )
    ...
    def get_full_name(self):
        ...
        if self.academic_title:
            title = (
                " " + str(self.academic_title)
                if title_last
                else str(self.academic_title) + " "
            )
        else:
            title = ""

        return "{}{}{}{}{}{}".format(
            title if not title_last else "",
            self.first_name + " " if self.first_name else "",
            self.initials + " " if not self.first_name else "",
            self.prefix + " " if self.prefix else "",
            self.last_name,
            title if title_last else "",
        )
    ...
    def save(self, *args, **kwargs):
        ...
        while count_s < MAXCOUNT and count_u < MAXCOUNT and count < MAXCOUNT:
            try:
                ...
                if "UNIQUE constraint failed: alumni_alumnus.username" in str(e) or (
                    "Duplicate entry" in str(e) and "for key 'username'" in str(e)
                ):
                    count_u += 1
                    self.username += "_{0}".format(count_u)
                if "UNIQUE constraint failed: alumni_alumnus.slug" in str(e) or (
                    "Duplicate entry" in str(e) and "for key 'username'" in str(e)
                ):
                    count_s += 1
