...
class Entry(models.Model):
    ...
    title = models.CharField(_('title'), max_length=120, default="Practicum")
    dome = models.PositiveSmallIntegerField(_('dome'), choices=DOME_CHOICES,
                                            default=1)
...
#   date = models.DateField(_('date of observing'), default=datetime.date.today)
    date = models.DateField(_('date of observing'))
    date_end = models.DateField(
        _('end date of observing'), blank=True, null=True,
        help_text=_("End date of the observations, if different from the "
                    "start date"))
    creator = models.ForeignKey(User, related_name='apo_creator')
    slug = models.SlugField(_('slug'), max_length=500, blank=False,
                            editable=False, unique=False)


    def save(self, *args, **kwargs):
        self.slug = '{}_{}-{}-{}_{}'.format(self.dome, self.date.year,
