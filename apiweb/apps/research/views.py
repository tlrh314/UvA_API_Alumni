...
def thesis_list(request):
    ...
    if sort_on:
        ...
        if sort_on[0] == "msc_lh":
            theses = (
                theses.filter(type__iexact="msc").distinct().order_by("date_of_defence")
            )
        if sort_on[0] == "msc_hl":
            theses = (
                theses.filter(type__iexact="msc")
                .distinct()
                .order_by("-date_of_defence")
            )

        if sort_on[0] == "phd_lh":
            theses = (
                theses.filter(type__iexact="phd").distinct().order_by("date_of_defence")
            )
        if sort_on[0] == "phd_hl":
            theses = (
                theses.filter(type__iexact="phd")
                .distinct()
                .order_by("-date_of_defence")
            )
