    ...
    if has_results:
        ...
        for j in range(len(api_record_set)):
            ...
            try:
                ...
                master_program = master_program_el.text.split('</td>')[0].split(':')[1].strip(' \t\n\r')[0:]

            date =  date_el.text.split('</td>')[0].strip('\t\n\r')
            research_group =  research_group_el.text.split('</td>')[0].split(':')[1].strip('\t\n\r')[1:]
            supervisor = supervisor_el.text.split('</td>')[0].split(':')[1].strip('\t\n\r')[1:]
            ...
            #format the info
            thesis_title = description_el.find('strong').text.split('</strong>')[0].strip('\t\n\r')
            description_text = str(description_el).split('</td>')[-2].split('<br/>')[-1].strip('\n\t\r')
            ...
            if download_abstract:
                if not abstract_link == not_found_name:
                    ...
                    if not os.path.exists(target_filename_full):
                        downloaded_filename = wget.download(abstract_link, out=target_filename_full)
            ...
            if download_fullPdf:
                if not fulltext_link == not_found_name:
                    ...
                    if not os.path.exists(target_filename_full):
                        downloaded_filename = wget.download(fulltext_link, out = target_filename_full)
                        if os.path.exists(target_filename_full):
            ...
            if download_profilePicture:
                if not profilePicture_link == not_found_name:
                    ...
                    target_filename_full = str(file_target_location+profilePicture_filename)
                    if not os.path.exists(target_filename_full):
                        downloaded_filename = wget.download(profilePicture_link, out = target_filename_full)
                        if os.path.exists(target_filename_full):
            ...
            if download_thesisPicture:
                if not thesisPicture_link == not_found_name:
                    ...
                    target_filename_full = str(file_target_location+thesispicture_filename)
                    if not os.path.exists(target_filename_full):
                        downloaded_filename = wget.download(thesisPicture_link, out=target_filename_full)
                        if os.path.exists(target_filename_full):
            ...
            supervisor_initials, supervisor_prefix, supervisor_last_name = split_name(supervisor_name)
...
def loop_over(file_target_location):
    ...
    while has_results == True:
        ...
        has_results, sub_result_list = get_persons(base_url, i, degree_type, True, file_target_location)
